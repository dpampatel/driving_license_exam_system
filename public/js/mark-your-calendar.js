...
(function ($) {
  ...
  $.fn.markyourcalendar = function (opts) {
    ...
    this.hideSlots = function (p_date) {
      if (settings.hideSlots) {
        return settings.hideSlots(...arguments);
      }
      return false;
    };
    this.setSlotTaken = function (p_date, time) {
      if (settings.setSlotTaken) {
        return settings.setSlotTaken(...arguments);
      }
      return "";
    };
    this.setSelections = function (p_date, p_time) {
      let l_filter = settings.selectedObjects.filter(function (item) {
    ...
    this.getAvailableTimes = function () {
      ...
      for (i = 0; i < 7; i++) {
        ...
        $.each(settings.availability[i], function () {
          let currentDate = settings.startDate.addDays(i);
          if (instance.hideSlots(currentDate)) return;
          tmpAvailTimes +=
            `<a href="javascript:;" class="myc-available-time` +
            instance.setSlotTaken(currentDate, this) +
            instance.setSelections(currentDate, this) +
            `"data-time="` +
            this +
            `"data-date="` +
            currentDate.getTime() +
    ...
    this.on("click", ".myc-available-time", function () {
      ...
      if ($(this).hasClass("selected")) {
        ...
        if (idx !== -1) {
          ...
          settings.selectedObjects.splice(idx, 1);
        }
      } else {
        if (settings.isMultiple) {
          $(this).addClass("selected");
          settings.selectedDates.push(tmp);
          settings.selectedObjects.push(l_obj);
        } else {
          settings.selectedDates.pop();
          settings.selectedObjects.pop();
